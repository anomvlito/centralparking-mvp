...
- [ ] Existe una lista durable de patentes excluidas con distancia máxima
  configurable; la patente real no aparece en código, docs, fixtures ni logs.
- [ ] Una detección exacta o a distancia Levenshtein 1 de una exclusión queda
  `DISMISSED`, conserva evidencia y no genera propuesta ni estadía.
- [ ] Un backfill idempotente descarta pendientes históricos alcanzados.
- [ ] Una pareja con duración calculada de 0 minutos no crea una estadía útil:
  queda `VOID`, sus detecciones `DISMISSED` y se audita como duplicado.
- [ ] Un backfill reversible anula sesiones de 0 minutos ya existentes sin
  borrar fotos o filas.
- [ ] Todas las imágenes del Dashboard se pueden pulsar para abrir un visor
  ampliado y cerrar con botón, fondo o `Escape`.
- [ ] La barra superior de conciliación muestra las fotos seleccionadas de
  entrada y salida antes de confirmar.
- [ ] Las evidencias se sirven con caché privada de navegador por una hora,
  sin convertirlas en contenido público ni perder el control de acceso.
- [ ] Exclusión, backfills y escrituras requieren administrador.
